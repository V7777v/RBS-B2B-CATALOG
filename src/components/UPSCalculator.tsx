...
const rawProducts = [
  ...
  { id: "p68", hebName: "אל-פסק אונליין 1KVA (מגדל)", watts: 900, category: "ups-online", internalWh: 216, performanceMatrix: { 0: [[450,10],[675,4],[900,2]], 1: [[450,40],[675,20],[900,15]], 2: [[450,85],[675,50],[900,35]], 3: [[450,120],[675,75],[900,55]], 4: [[450,180],[675,105],[900,75]] }, extPack: { model: "DS-UPSB0924B-R/TC(O-STD)/EU", maxPacks: 4, image: "https://assets.hikvision.com/prd/normal/all/image/m000162284/2.png?f=webp", link: "https://drive.google.com/file/d/1K9kP8_lLh-YWBpoYnKIhdQ3dzIeK_gXK/view?usp=sharing" }, canExtend: true, model: "DS-UPS01K24-R/TS(O-STD)/EU/IEC", image: "https://assets.hikvision.com/prd/normal/all/image/m000162281/1k-%E5%B7%A6%E5%89%8D%E4%BE%A7.png?f=webp", link: "https://drive.google.com/file/d/14ZzKUKJ-zbsJzQs1s35h6fWYt9Xy6SRQ/view?usp=sharing" },
  { id: "p69", hebName: "אל-פסק אונליין 2KVA (מגדל)", watts: 1800, category: "ups-online", internalWh: 432, performanceMatrix: { 0: [[900,12],[1350,5],[1800,2.2]], 1: [[900,44],[1350,22],[1800,17.7]], 2: [[900,86],[1350,52],[1800,35]], 3: [[900,120],[1350,77],[1800,54]], 4: [[900,183],[1350,106],[1800,75.4]] }, extPack: { model: "DS-UPSB0948B-R/TC(O-STD)/EU", maxPacks: 4, image: "https://assets.hikvision.com/prd/normal/all/image/m000162301/6.png?f=webp", link: "https://drive.google.com/file/d/1K9kP8_lLh-YWBpoYnKIhdQ3dzIeK_gXK/view?usp=sharing" }, canExtend: true, model: "DS-UPS02K48-R/TS(O-STD)/EU/IEC", image: "https://assets.hikvision.com/prd/normal/all/image/m000162282/2k%263k-%E5%B7%A6%E5%89%8D%E4%BE%A7.png?f=webp", link: "https://drive.google.com/file/d/14ZzKUKJ-zbsJzQs1s35h6fWYt9Xy6SRQ/view?usp=drive_link" },
  { id: "p70", hebName: "אל-פסק אונליין 3KVA (מגדל)", watts: 2700, category: "ups-online", internalWh: 648, performanceMatrix: { 0: [[1350,10],[2025,5.5],[2700,2.7]], 1: [[1350,45],[2025,23],[2700,19]], 2: [[1350,87],[2025,55],[2700,35.7]], 3: [[1350,122],[2025,78],[2700,55]], 4: [[1350,185],[2025,108],[2700,75]] }, extPack: { model: "DS-UPSB0972B-R/TC(O-STD)/EU", maxPacks: 4, image: "https://assets.hikvision.com/prd/normal/all/image/m000162302/6.png?f=webp", link: "https://drive.google.com/file/d/1K9kP8_lLh-YWBpoYnKIhdQ3dzIeK_gXK/view?usp=sharing" }, canExtend: true, model: "DS-UPS03K72-R/TS(O-STD)/EU/IEC", image: "https://assets.hikvision.com/prd/normal/all/image/m000162283/2k%263k-%E5%B7%A6%E5%89%8D%E4%BE%A71.png?f=webp", link: "https://drive.google.com/file/d/14ZzKUKJ-zbsJzQs1s35h6fWYt9Xy6SRQ/view?usp=drive_link" },
...
export const UPSCalculator: React.FC<{ catalogData?: any[], onAddToCart?: (product: any, quantity?: number) => void, onAskAdvisor?: (question: string) => void }> = ({ catalogData = [], onAddToCart, onAskAdvisor }) => {
  ...
  if (tech === 'online_std') upsProducts = upsProducts.filter(p => p.hebName.includes('(מגדל)'));
  if (tech === 'online_ext') upsProducts = upsProducts.filter(p => p.hebName.includes('(ארון)'));
                     ...
                     <div className="bg-[#f8f9fa] rounded-lg border border-gray-100 overflow-hidden mt-1 relative z-10">
                       ...
                       <div className="p-2 space-y-1.5">
                         {r.configs.map((c: any, ci: number) => {
                           const isTower = (r.hebName || '').includes('(מגדל)');
                           let label = c.packs === 0 ? 'המכשיר עצמו (ללא הרחבה)' : (r.internalWh === 0 && c.packs === 1 ? 'עם 1 מארז חיצוני (מינימום נדרש)' : `הוספת ${c.packs} מארזי סוללות ${isTower ? '(Tower)' : 'לארון'}`);
                           return (
