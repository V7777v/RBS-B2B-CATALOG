...
const tSoon = `{product.isComingSoon && (
...
const tNew = `{product.isNew && !product.isComingSoon && (
...
const targetTextContainer = `<div className="p-3 sm:p-4 flex flex-col flex-grow text-center relative">`;

const rSoon = `{product.isComingSoon && (
...
const rNew = `{product.isNew && !product.isComingSoon && (
...
if (content.includes(tSoon) && content.includes(tNew)) {
    ...
    console.log('SUCCESS ProductCard');
} else {
    console.log('FAILED to find ProductCard badges');
}
