...
serverCode = serverCode.replace(
  /function stripSensitiveColumnsServer[\s\S]*?return Papa\.unparse\(out\);\n}/, 
  replacementFunctions
);

// Now update the route handler
const proxyRoutePattern = /let csvString = await fetchSheetDataV4\(String\(gid\), limit as string, offset as string\);\s*if \(!authorized\) \{\s*csvString = stripSensitiveColumnsServer\(csvString, String\(gid\)\);\s*\}/;

const newProxyRoute = `let csvString = await fetchSheetDataV4(String(gid), undefined, undefined);
...
serverCode = serverCode.replace(proxyRoutePattern, newProxyRoute);
