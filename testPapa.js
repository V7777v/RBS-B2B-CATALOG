      ...
      complete: (results) => {
        ...
        results.data.forEach((row, i) => {
           ...
           if (p && p.includes('.') || p.includes(',')) {
              if (parsed === 0) {
                 console.log(`!! HAS DOT/COMMA BUT ZERO: Row ${i+2}: "${p}"`);
              }
           }
        });
        ...
        let decimals = results.data.filter(r => r.price && (r.price.includes('.') || r.price.includes(','))).slice(0, 10);
        console.log("\\nSample decimal prices in CSV:");
        decimals.forEach(r => console.log(`"${r.price}" => ${parsePrice(r.price)}`));
      }
    });
  });
});
